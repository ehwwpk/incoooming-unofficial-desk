...
def build_open_risk_summary(snapshot: DashboardSnapshot) -> OpenRiskSummary | None:
    ...
    if live_book is not None:
        options = tuple(
            _live_input(option, fallback_observed_at=snapshot.as_of)
        ...
        equities = tuple(
            UnderlyingEquityRiskInput(
                ...
                underlying_price=item.current_price or Decimal(0),
            )
        ...
        equities = tuple(
            UnderlyingEquityRiskInput(
                ...
                shares=Decimal(underlying.shares),
...
def _live_input(
    ...
    *,
    ...
    multiplier = abs(option.contract_multiplier or STANDARD_MULTIPLIER)
    return OpenOptionRiskInput(
        ...
        deliverable_share_quantity=multiplier,
        strike=option.strike,
        underlying_price=option.underlying_price or Decimal(0),
...
def _clock_input(
    ...
    *,
    ...
    return OpenOptionRiskInput(
        ...
        premium_multiplier=STANDARD_MULTIPLIER,
        deliverable_share_quantity=STANDARD_MULTIPLIER,
        strike=clock.strike,
        ...
        quote_quality=(
            ...
            if simulated
            else _quote_quality(clock.quote_status)
        ),
