...
def calculate_option_economics(
    *,
    ...
    opening_credits = sum(
        (_gross(row) for row in in_window if _is_opening_sale(row)), ZERO
    )
    closing_debits = sum(
        (_gross(row) for row in in_window if _is_closing_buy(row)), ZERO
    )
    ...
    closed = tuple(
        ...
        and _inside(campaign.closed_on, coverage_start, coverage_end)
    )
    exact = sum(
        campaign.confidence
        in {CampaignLinkConfidence.EXACT, CampaignLinkConfidence.USER_CONFIRMED}
    ...
    return OptionEconomics(
        ...
        closed_campaign_result=sum((item.net_cash_to_date for item in closed), ZERO),
        closed_campaigns=len(closed),
        exact_closed_campaigns=exact,
        inferred_closed_campaigns=inferred,
        open_mark_profit_loss=sum(open_mark_values, ZERO) if open_mark_values else None,
        current_option_liability=sum(liabilities, ZERO) if liabilities else None,
        ...
        method_note=(
            ...
            "finished inside the same return window; open mark P/L remains unrealized. Slippage "
            "needs execution-time bid/ask snapshots and is not estimated."
        ),
    )
...
def _latest_position_rows(rows: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], ...]:
    ...
    for row in rows:
        ...
        if previous is None or previous.get("observed_at") is None or (
            ...
            latest[account] = row
    latest_keys = {
        (str(row.get("account_mask") or "ACCOUNT"), row.get("observed_at"))
...
def _is_opening_sale(row: dict[str, Any]) -> bool:
    return str(row.get("side")) == "sell" and str(row.get("position_effect")) == "opening"


def _is_closing_buy(row: dict[str, Any]) -> bool:
