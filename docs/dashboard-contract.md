...
- Live `DAY P/L` is the sum of every open position's normalized Schwab day print (or the imported day column) on the latest completed sync, across every asset type and account in that snapshot. Schwab can include same-session trade cash in `currentDayProfitLoss`; ingestion removes `currentDayCost` so a purchase is not shown as a market loss and an opening sale is not shown as a market gain. The untouched broker payload remains in the raw-event audit trail. Percent is those dollars divided by implied prior account value (`NET LIQ` or net position value minus the tape). Deposits and withdrawals are not part of this chip. If a book has positions but none of them carry a day print, the chip is blank rather than a false zero. Results time-weighted return remains on the liquidation path and is a different identity.
