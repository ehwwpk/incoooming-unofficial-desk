...
- Live `ACCOUNT DAY P/L` is the latest linked change in aggregate broker net liquidation minus owner deposits and withdrawals between the two market-session snapshots. Its percentage is the same daily time-weighted return used by Results, so the desk header and Results cannot disagree about the sign of the day. The value is unavailable when fewer than two sessions exist or account coverage changes; it never falls back to a partial position sum.
