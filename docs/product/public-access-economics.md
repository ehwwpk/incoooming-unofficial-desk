...
| Mapped broker CSV | Exports a recognized file and imports it | Can be excellent for executed history when the exact surface is supported; not live and formats vary | Near-zero variable cost | Portable fallback and historical backfill, with preview and fail-closed validation |
